...
class Prompt_Text:
    ...
    def show(self):
        ...
        rect = pygame.rect.Rect((0, 0), prompt_text_font.size(self.text))
        ...
        sur.fill((0, 0, 0))
        sur.blit(prompt_text_font.render(self.text, True, (0, 0, 0), (255, 255, 255)), (2, 2))
        screen.blit(sur, rect2.topleft)
def show_prompt_text(text = None, pos = None, show = False):#显示提示文字，event == 1表示开始显示，event == 0表示停止显示，event == 其他数表示继续显示
    global prompt_text_list
    if show and prompt_text_list != []:
        for p in prompt_text_list:
            p.show()
        prompt_text_list = []
    else:
        if text != None:
            ...
            prompt_text_list.append(Prompt_Text(text, pos))
def select_file(mode, initial_path):
    '''
    :param mode: 0:save, 1:open
    :return: file name or None
    '''
    window = win32ui.CreateFileDialog(mode)
    window.SetOFNInitialDir(initial_path)
    if window.DoModal() != 1:
        return None
    else:
        return window.GetPathName()
class Text:
...
class Scale:
    def __init__(self, surface, pos, points, width, enable=True, direction='x', start_point=0, name=None, cross=False, parts=None, mouse_on_callback=None, adj_callback=None, text=None, prompt_text=None,
        ...
        if adj_callback == None:
            self.adj_callback = space.adj_parameters_of_shape
        else:
...
class Toolbar:#工具栏
    ...
    def play_sound(self):
        ...
        self.sound.play()
class Menu:
    def __init__(self, surface, menu_cfg_input, close_callback=None):
        menu_cfg = menu_cfg_input.copy()
